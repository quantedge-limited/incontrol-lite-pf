...
// Remove the duplicate fetchPublicProducts function

export const productApi = {
  // Get products for customers (public endpoint)
  async getCustomerProducts(params?: {
    category?: number;  // Change from string to number (category ID)
    search?: string;
  }): Promise<FrontendProduct[]> {
    try {
      // Use inventoryApi.getProducts() - it doesn't check auth now
  ...
  async getCustomerProduct(id: number): Promise<FrontendProduct> {
    try {
      // NOTE: This will fail because inventoryApi.getProduct() checks auth!
      // You need to update inventoryApi.getProduct() to have a public version too
  ...
  async getCustomerCategories(): Promise<any[]> {
    try {
      // NOTE: This will fail because inventoryApi.getCategories() checks auth!
      // You need a public version of getCategories too
      const API_BASE = process.env.NEXT_PUBLIC_API_BASE_URL || 'https://incontrol-lite-pb.onrender.com/api';
